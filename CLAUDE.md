...
- **독서 점수는 캡 없이 "쓴 만큼 그대로"**: 데일리 read = 그날 감상문 편수, 주간·스트릭·순위도
  ...
  삭제 시 통계 자동 차감.
- **독서는 2026-07-05부터 상시 누적(베타 초기화 제외 대상)**: 방학(개학 8/17 전) 감상문은
  `readingStats.byWeek["0"]` 버킷 — 총권수·마라톤·개인 누적 점수(+1/편, `payVacationReading`이
  `_cumulative.vacReadPaid` 마커 델타로 지급, 초기화 후 자동 복원)에만 반영되고, 주간
  스트릭·모둠 대항·주간 최다 독서 모둠 보상은 개학(1주차)부터. 베타 초기화는 7/5 이전 연습
  감상문만 지우고 통계를 재구축한다(`betaReset.ts` READING_KEEP_FROM).
