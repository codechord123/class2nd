...
export default function ReadingAdjustPanel() {
  ...
  // 실물 진단 — 버튼을 눌렀을 때만 감상문 전체를 1회 읽어 통계와 다른 학생을 찾는다.
  // (읽기 예산 보호: 탭 진입마다 자동 조회하면 감상문이 쌓일수록 읽기 비용이 커진다)
  const [diag, setDiag] = useState<{ id: number; stat: number; real: number }[] | null>(null);
  async function runDiagnose() {
    ...
    try {
      ...
      snap.forEach((docu) => {
        const v = docu.data();
        if (v.isDraft) return;
        c[String(v.studentId)] = (c[String(v.studentId)] ?? 0) + 1;
      });
      setDiag(
        students
          .map((s) => ({ id: s.id, stat: stats?.total?.[String(s.id)] ?? 0, real: c[String(s.id)] ?? 0 }))
          .filter((m) => m.stat !== m.real)
      );
    } catch (e) {
      toast(`⚠️ 진단 실패: ${e instanceof Error ? e.message : String(e)}`, "error");
    } finally {
      setBusy(false);
    }
  }
  ...
  async function recount() {
    ...
    try {
      ...
      await qc.invalidateQueries({ queryKey: ["readingStats"] });
      setDiag([]); // 재계산 후 실물과 일치 (다음 진단까지 초록 표시)
      toast(
        changed.length
  ...
  return (
    <section className="rounded-card border border-ink-200 bg-white p-4 shadow-card">
      ...
      {/* 유령 권수 진단 + 정리 — '진단하기'를 누를 때만 감상문을 읽어 비교 (읽기 예산 보호) */}
      <div className="mt-3 border-t border-ink-100 pt-3">
        {diag && diag.length > 0 && (
          <div className="mb-2 rounded-btn bg-rose-50 p-3">
            <p className="text-sm font-bold text-rose-600">
              ⚠️ 2학기 권수가 실제 감상문 수와 다른 학생 {diag.length}명
            </p>
        ...
        <div className="flex flex-wrap items-center gap-2">
          <button
            ...
            disabled={busy}
            className="press rounded-btn border border-ink-300 bg-white px-3 py-2 text-sm font-bold text-ink-600 disabled:opacity-40"
          >
            🔍 유령 권수 진단
          </button>
          <button
            ...
            className={`press rounded-btn px-3 py-2 text-sm font-bold disabled:opacity-40 ${
              diag && diag.length > 0 ? "bg-rose-500 text-white" : "border border-warn/50 bg-white text-warn"
            }`}
          >
            🔄 실물 기준 재계산
          </button>
          <span className="text-[11px] text-ink-400">
            먼저 진단으로 어긋난 학생을 확인하고, 재계산으로 실제 감상문 수에 맞춰요.
          </span>
        </div>
      </div>
    </section>
