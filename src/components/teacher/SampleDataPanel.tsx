...
export default function SampleDataPanel({ date }: { date: string }) {
  ...
  async function generate() {
    ...
    const ok = await confirm({
      title: "샘플 평가를 만들까요?",
      body: `${date} 날짜로 25명 전원의 평가·MVP·칭찬·건의 샘플을 만들고 바로 집계해요.\n리포트가 실제 모습으로 채워집니다. (아래 '샘플 지우기'로 되돌릴 수 있어요)`,
      confirmLabel: "만들기",
    });
    if (!ok) return;
    setBusy(true);
    try {
      ...
      refresh();
      toast(
        `🎲 샘플 ${r.entries}명 생성·집계 완료! 순위: ${r.ranking.map((g, i) => `${i + 1}위 ${g}모둠`).join(" ")}`,
        "success"
      );
    } catch (e) {
  ...
  async function clear() {
    ...
    const ok = await confirm({
      title: "샘플을 지울까요?",
      body: `${date} 날짜의 평가 기록을 모두 삭제하고 재집계해요. 누적 점수도 원래대로 돌아갑니다.`,
      confirmLabel: "지우기",
      danger: true,
    });
    if (!ok) return;
