...
//  · 선생님 순위(groupRank)·칭찬 미션(mission)·페어플레이(fair)는 모둠 단위 사건 — 모둠당 1회만
//    (페어플레이는 사용자 확정으로 모둠 점수에도 포함 — 동점 다수여도 1회만 세서 부풀림 방지)
...
export interface GroupDayScore {
  ...
  fairOnce: number; // 🤝 페어플레이 (모둠당 1회 — 동점 다수여도 1회)
  read: number; // 독서 합
...
export function groupDayScore(
  ...
  for (const id of memberIds) {
    ...
    if (!s.fairOnce && r.fair) s.fairOnce = r.fair;
    s.read += r.read ?? 0;
    s.bonus += r.bonus ?? 0;
    s.peer += r.peer ?? 0;
    s.comp += r.comp ?? 0;
  ...
  s.total = s.rankOnce + s.missionOnce + s.fairOnce + s.read + s.bonus;
  return s;
}
